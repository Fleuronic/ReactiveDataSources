...
fileprivate extension AnimatableSectionModelType {
    init(safeOriginal: Self, safeItems: [Item]) throws {
        ...
        if self.items != safeItems || self.id != safeOriginal.id {
...
public enum Diff {
    ...
    private struct OptimizedID<ID: Hashable> : Hashable {

        let id: UnsafePointer<ID>
        private let cachedHashValue: Int

        init(_ id: UnsafePointer<ID>) {
            self.id = id
            self.cachedHashValue = id.pointee.hashValue
        }
        ...
        static func == (lhs: OptimizedID<ID>, rhs: OptimizedID<ID>) -> Bool {
            if lhs.hashValue != rhs.hashValue {
                return false
            }

            if lhs.id.distance(to: rhs.id) == 0 {
                return true
            }

            return lhs.id.pointee == rhs.id.pointee
        }
    ...
    private static func calculateAssociatedData<Item: Identifiable>(
        ...
        -> (ContiguousArray<ContiguousArray<ItemAssociatedData>>, ContiguousArray<ContiguousArray<ItemAssociatedData>>) {
            ...
            var initialIdentities: ContiguousArray<ID> = ContiguousArray()
            ...
            try initialIdentities.withUnsafeBufferPointer { (identitiesBuffer: UnsafeBufferPointer<ID>) -> Void in
                var dictionary: [OptimizedID<ID>: Int] = Dictionary(minimumCapacity: totalInitialItems * 2)

                for i in 0 ..< initialIdentities.count {
                    let idPointer = identitiesBuffer.baseAddress!.advanced(by: i)

                    let key = OptimizedID(idPointer)
